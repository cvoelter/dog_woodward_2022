...
first4s.data.first.fix.nolooked.included.se.long <- first4s.data.first.fix.nolooked.included.agg %>%
  ...
  pivot_longer(cols = c(first_fix_old_goal, first_fix_old_side), names_to = "IA", values_to = "se.dogs")
...
first4s.data.first.fix.nolooked.included.agg.long <- first4s.data.first.fix.nolooked.included.agg %>%
  ...
  pivot_longer(cols = c(first_fix_old_goal, first_fix_old_side), names_to = "IA", values_to = "prop.dogs") %>%
  full_join(first4s.data.first.fix.nolooked.included.se.long)

al_plot_NAs_included2<-ggplot(first4s.data.first.fix.nolooked.included.agg.long, aes(x = trialtype_agent, y = prop.dogs, fill = IA)) +
...
library(ggbeeswarm)
ggplot(first4s.data.latency.first.fix.long.agg, aes(x = trialtype_agent, y = mean_latency, fill = IA)) +

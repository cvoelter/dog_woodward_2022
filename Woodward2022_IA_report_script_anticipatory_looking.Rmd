...
#old plot without lines connecting subjects
latency_AL_AOIs_plot

#new plot with lines connecting subjects
latency_AL_AOIs_plot_lines<-ggplot(first4s.data.latency.first.fix.long.agg, aes(x = trialtype_agent, y = mean_latency, fill = AoI)) +
...
new identity"))+
  ...
  ylim(-3800,250)+
  ylab("Mean latency to first fixation")+
  xlab("Agent")+
...
latency_AL_AOIs_plot_lines
```
```{r}
ggsave(filename="graphs/latency_AL_AOIs2_lines.png",
  plot = latency_AL_AOIs_plot_lines,
  width=10,
  height=6,
  scale=0.55,
  dpi=1200)

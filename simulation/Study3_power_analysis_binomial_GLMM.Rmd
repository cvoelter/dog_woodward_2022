---
title: "Study 3 - Goggles - binomial GLMM - power simulation"
...
clear.per <- c(0.35, 0.4) # performance in clear condition
opaque.per <- c(0.6, 0.65) # performance in opaque condition

# add condition and trial number
start.data <- data.frame(subj.id = c(1:n.subject))
                         
start.data$order = rep(c("clear_opaque_opaque_clear", "clear_opaque_clear_opaque", "opaque_clear_clear_opaque", "opaque_clear_opaque_clear"), n.subject/n.per.subject)
...
start.data$sex<-rep(c(rep("f",16), rep("m", 16)), n.subject/8)
...
r.effects <- c(0.4) # random effects to be simulated
# with the intercept being -2.2 (qlogis(0.4)) we could make the following
# guesses for the random intercept:
#- 0.4: moderate random intercepts effect


r.slope.con <- c(0.8)
# with the intercept being -0.8 (qlogis(0.4)-qlogis(0.6)) we could make the following
...
for (i in 1:nrow(all.res)) {
  ...
  # fit full model:
  full <- keepWarnings(glmer(correct ~ condition + z.age + sex + z.trial + (1 + condition.c + z.trial|| subj.id),
    data = xdata, family = binomial, control = contr
  ))
  # fit null model:
  null <- keepWarnings(glmer(correct ~ z.trial + (1 + condition.c + z.trial|| subj.id),
    data = xdata, family = binomial, control = contr
  ))

  # store results:
  all.res[i, c("icpt", "conditionopaque", "z.age", "sexm", "z.trial")] <- fixef(full$value)
...
summary(full$value)
save.image("power_sim_study3_N64.RData")
```
...
ggsave( p.con.power, filename = "Study3_power.png", scale = 0.5, height = 8, width = 9)
```

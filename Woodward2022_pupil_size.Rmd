...
baseline.end<-4000 #frozen frame with agent centered between objects (IP time, in ms)
#agents' directional cue finishes around frame 493 (4930 ms)
...
plot.woodward.pupil.group <- ggplot(data = pupil.group.level, aes(x = bin_low, y = mean.pupil.corrected.binned)) +
  ...
  xlim(3000, 8001) +
    theme_bw()+
  scale_color_manual(values = wes_palette("Zissou1")[c(1, 4)],
                     labels = c("New identity", "New side")) +
  scale_fill_manual(values = wes_palette("Zissou1")[c(1, 4)],
                    labels = c("New identity", "New side")) +
...
ggsave(plot.woodward.pupil.group, filename = "graphs/pupil_group_condition.png", height = 5, width = 10, scale = 0.7, dpi=600)
```

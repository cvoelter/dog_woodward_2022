...
```{r}
sample.data<-sample.data%>% filter(subject != "Juna" | trial_number != "2") %>%
...
exp.data.pupil.processed <- sample.data %>%
  ...
  #LL: now that I excluded the two trials with less than 70% tracked data,
  #an error occurs that prevents linear interpolation
...
```
# Plot group level data (condition)
```{r}
...
gammm.int <- bam(pupil_base ~ condition*agent + 
           ...
           s(Xgaze, Ygaze) +
           s(time, Event, bs='fs', m=1), #LL: is time needed here or is Event alone sufficient?
...
compareML(gamm.null, gammm.int) #AIC difference: 29.12, model gammm.int has lower AIC.#LL: is it ok that there is no chisq test for the difference in ML scores? 
...
save.image("C:/Users/lonardol/Desktop/Eye-tracker/Action_understanding_experiments/Kano_Woodward/Woodward_2022/pupil_size.RData")
...
dev.off()
#LL: the scatterplot of residuals shows a value that is more frequent than others (larger size of one of the dots compared to all others)? 
...
summary(gamm.full)
###Fixed effects:
#in the new side condition, the pupil is in general larger than in the new goal condition, irrespective of the agent shown (t=2.57, P=0.010).
#Agent did not have a significant effect (t=1.77, P=0.077)
```

...
baseline.end<-4000 #first 4 seconds of video, frozen frame with agent centered between objects (IP time, in ms)
#agents' directional cue at frame 493 (4930 ms)
...
mean(prop.tracked.gaze.data$prop)
...
puphist <-
  ggplot(
    sample.data %>% filter(subject != "Juna" | num != "2") %>%
                    filter(subject != "Maylo" | num != "3"),
...
```{r}
sample.data<-sample.data%>% filter(subject != "Juna" | num != "2") %>%
                            filter(subject != "Maylo" | num != "3") #exclude the 2 test trials in which the eye was not tracked for more than 30% of the trial time
...
#ggsave(plot.woodward.pupil.individual, filename = "graphs/pupil_processed_baseline_corrected_individual.png", height = 7, width = 12, scale = 0.8)
```
# Plot group level data (conditions)
```{r}
...
#ggsave(plot.woodward.pupil.group, filename = "graphs/pupil_group_condition.png", height = 5, width = 8, scale = 0.7, dpi=600)
```
#Plot group level data (agent)
```{r}
plot.woodward.pupil.group.agent <- ggplot(data = pupil.group.level, aes(x = bin_low, y = mean.pupil.corrected.binned)) +
  geom_vline(aes(xintercept=baseline.end), lty=2, alpha=0.3)+
    ylab("Pupil size") +
  xlab("Time (in ms)") +
  geom_point(aes(x = bin_low, y = mean.pupil.corrected.binned, color = agent), alpha = 0.9, size = 0.5) +
  geom_ribbon(aes(ymin = mean.pupil.corrected.binned - se.pupil.corrected.binned, ymax = mean.pupil.corrected.binned + se.pupil.corrected.binned, fill = agent), alpha = 0.7) +
    xlim(0, 22000) +
    theme_bw()+
  scale_color_manual(values = wes_palette("Zissou1")[c(1, 4)],
                     labels = c("Human", "Inanimate")) +
  scale_fill_manual(values = wes_palette("Zissou1")[c(1, 4)],
                    labels = c("Human", "Inanimate")) +
  guides(colour = guide_legend(override.aes = list(size=2)))+
  theme(panel.grid.major.x = element_blank(),
...
plot.woodward.pupil.group.agent

ggsave(plot.woodward.pupil.group.agent, filename = "graphs/pupil_group_agent.png", height = 5, width = 8, scale = 0.7, dpi=600)
```
...
dat <- exp.data.pupil.processed.downsampled%>%
  filter(bin_low >= 5000 & bin_low <= 9000)%>% #select interest period
...
library(mgcv)
gammm.int <- bam(pupil_base ~ condition*agent + 
           s(time, k=20) + 
           ...
           s(Xgaze, Ygaze) +
           s(time, Event, bs='fs', m=1), #is time needed here or is Event alone sufficient?
...
compareML(gamm.null, gammm.int) #AIC difference: 29.12, model gammm.int has lower AIC.#is it ok that there is no chisq test? 
# "Chi-square test of ML scores
# -----"?

AIC(gammm.int, gamm.null)

summary(gammm.int)#linear interaction not significant, we remove it
...
#LL: the pupil size workspace is too large for GitHub, hence I save it on my laptop
save.image("~GitHub/dog_woodward_2022/workspaces/pupil_size.RData")

##########17/04?2023: continue from here
...
#save gam_check
library(visibly)
png("graphs/GAMM01_gam_check.png", units="cm", res=600, width=24, height = 18)
gam_check_plot<-plot_gam_check(m2, scatter=TRUE)
print(gam_check_plot)
dev.off()

png("graphs/GAMM01_acf_plot.png", units="cm", res=600, width=12, height = 12)
...
ggsave(plot_diff_m2_tr1_tr2, filename = "graphs/frisbee_triangle_pupilsize_diff_m2.png", width=8, height=6, scale=0.6, dpi=1200)

```

* plotting partial effects
```{r}
pdf("graphs/frisbee_triangle_m2_GAMM_partial_effect.pdf", width=10, height = 8)
plot(m2, pages = 1, all.terms = TRUE, residuals = TRUE, rug=FALSE, pch = 1, cex = 0.5, seWithMean = TRUE, shade = TRUE, shade.col = "lightblue")
dev.off()

library(mgcViz)
b <- getViz(m2)
png("graphs/frisbee_triangle_m2_GAMM_partial_effect2.png", units="cm", res=600, width=24, height = 16)

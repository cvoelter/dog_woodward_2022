...
levels(as.factor(fam.data$IA_LABEL))
  ...
  #       "old_goal_new_side",
  #       ifelse(
  #         condition == "new_side" &          #refers to test conditions, excluded from this analysis
  #           ia_object != trialtype_object,
  #         "new_goal_old_side",
  #         ifelse(
  #           condition == "new_side" &
...
#LL: for some reason the levels of IA content are replaced by numbers (ball->1, elephant->2)
mutate(IA_content2 = fct_recode(IA_content2, "ball"="1", "elephant"="2")) %>% 
  mutate(IA_content3 = ifelse(IA_content2 == "agent", "agent",
...
fam.data.first.fix.wider <- fam.data.first.fix %>% 
  filter(!is.na(IA_FIRST_FIXATION_TIME)) %>% 
  pivot_wider(names_from = IA_content3, values_from = IA_FIRST_FIXATION_TIME) %>% 
  # group_by(subject, session_number) %>% 
  # filter(target>agent|distractor>agent) #does not work, excludes all data
  
############from AL during test script:##########  
  mutate(
    old_goal_new_side = replace_na(old_goal_new_side, 99999999),
    new_goal_old_side = replace_na(new_goal_old_side, 99999999)
  ) %>%
  mutate(
    first_fix_old_goal = as.numeric(ifelse(
      old_goal_new_side < new_goal_old_side, 1, 0
    )),
    first_fix_old_goal_withoutNA = as.numeric(ifelse(
      old_goal_new_side == new_goal_old_side,
      NA,
      ifelse(old_goal_new_side <
               new_goal_old_side, 1, 0)
    )),
    first_fix_old_side = as.numeric(ifelse(
      old_goal_new_side > new_goal_old_side, 1, 0
    )),
    first_fix_old_side_withoutNA = as.numeric(ifelse(
      old_goal_new_side == new_goal_old_side,
      NA,
      ifelse(old_goal_new_side >
               new_goal_old_side, 1, 0)

...
first4s.data.first.fix$z.session<-as.vector(scale(as.numeric(first4s.data.first.fix$session_number), center = TRUE, scale=TRUE))

first4s.data.first.fix$trialtype_agent <- as.factor(first4s.data.first.fix$trialtype_agent)
levels(first4s.data.first.fix$trialtype_agent)
first4s.data.first.fix$trialtype_agent.c=as.vector(scale(as.numeric(first4s.data.first.fix$trialtype_agent==levels(as.factor(first4s.data.first.fix$trialtype_agent))[2]), center=TRUE, scale= FALSE))
first4s.data.first.fix$agent_first.c=as.vector(scale(as.numeric(first4s.data.first.fix$agent_first==levels(as.factor(first4s.data.first.fix$agent_first))[2]), center=TRUE, scale= FALSE))
...
drop1(mm1_first_fix2, test="Chisq")
```
...
mm1_first_fix_withoutNA2_null=glmer(first_fix_old_goal_withoutNA ~ z.session+agent_first +
            (1|subject)+(0+trialtype_agent.c + z.session|subject),#removed the correlation between random intercept and random slope
             data=first4s.data.first.fix, family=binomial, 
            control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

anova(mm1_first_fix_withoutNA2, mm1_first_fix_withoutNA2_null, test ="Chisq")
summary(mm1_first_fix_withoutNA2)
drop1(mm1_first_fix_withoutNA2, test="Chisq")
```
## Plot showing the object looks in the first 4 s of the test trials (without dogs that did not look not at any of the objects in a given trial)
```{r}
...
## intercept only model (with agent first centered - to test whether the dogs have a preference for the new goal in the human condition)

```{r}
mm1_first_fix_intercept_only=glmer(first_fix_old_goal_withoutNA ~ trialtype_agent+z.session+agent_first.c +
            (1|subject)+(0+trialtype_agent.c + z.session|subject),#removed the correlation between random intercept and random slope
             data=first4s.data.first.fix, family=binomial, 
            control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
## intercept only model (with agent first centered and with relevelled agent reference category- to test whether the dogs have a preference for the new goal in the inanimate condition)

```{r}

first4s.data.first.fix$trialtype_agent.rel <- relevel(first4s.data.first.fix$trialtype_agent, ref = "inanimate") 
levels(first4s.data.first.fix$trialtype_agent.rel)
mm1_first_fix_intercept_only_agent_relevelled=glmer(first_fix_old_goal_withoutNA ~ trialtype_agent.rel+z.session+agent_first.c +
            (1|subject)+(0+trialtype_agent.c + z.session|subject),#removed the correlation between random intercept and random slope
             data=first4s.data.first.fix, family=binomial, 
            control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(mm1_first_fix_intercept_only_agent_relevelled)
```

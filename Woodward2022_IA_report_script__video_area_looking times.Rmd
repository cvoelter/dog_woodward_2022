...
full.trial.data <-
  read_delim(
    "data/Woodward2022_IA_Report_video_area_IP_full_trial_no_fix_trigger.txt",
    na = ".",
    delim = "\t"
  ) %>%
  dplyr::select(
...
#                                        ifelse(DATA_FILE == "Melody_7.edf" | DATA_FILE == "Mia_6.edf", session.num-3, session.num))) %>% #recoding of session number to account for dogs with aborted sessions
  ...
  full_join(demo.data)%>%
  mutate(proportion_video_area_lt = IA_DWELL_TIME / video_duration, proportion_trial_lt = TRIAL_DWELL_TIME / video_duration)

full.trial.data %>% filter (proportion_trial_lt < 0.7)

```


two dogs have trial dwell time values < 0.7 (Juna, Session 2, Maylo Session 3; in both cases in the test trials) but they did not seem to leave the chin rest.
Next steps: check these values using the sample report. 
...
############################################################################### LL: what follows is already in the script  "anticipatory looking", hence could be deleted from here, right?
```{r}
